...
| `AI_CLIENT_MODE=core` path | `backend/src/lib/ai/core-client.ts` | **Live-tested through this repo's actual proxy, 2026-08-11 (update: this was initially left as an open gap earlier in this same session, then closed later the same day — see below).** An initial check only confirmed `biassemble-core` itself (direct call, bypassing this repo). A follow-up test ran `next dev` locally with `AI_CLIENT_MODE=core` + the real `AI_CORE_API_KEY`, then `POST`/`GET`'d this repo's own `/api/grounnel/*` routes (not core directly) — full extract→poll→done cycle, real session write, real multi-claim result with both `diy_fetch` and `tavily_fallback` sources. `postCore`/`getCore` request-building, auth header, and `X-Grounnel-Client-IP` forwarding are now confirmed end-to-end, not just inferred. |

This plan's frontend work is a pure consumer of this surface; no backend PR is part of this feature. `specs/003-grounnel-backend/` now documents this proxy's own spec-kit trio, retroactively — see that directory for the full picture (including one real, separately-tracked gap: `AppException` mapping is incomplete for two error paths, `specs/003-grounnel-backend/tasks.md` T015).
