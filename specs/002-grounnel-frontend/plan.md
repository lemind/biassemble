...
**Performance**: Poll cadence ~10s (ADR-002 §5) — Grounnel's pipeline is a longer multi-claim process than the reflection product's single assessment call, not the same 2s cadence as `usePollAssessment`.

**Constraints**: No router dependency — single `window.location.pathname === '/grounnel'` branch in `App.tsx` (ADR-002 §3). No claim-offset data from the backend — highlighting is best-effort, not exact.
...
Spec only fixes green (`supported`) and red (`contradicted`); the rest is this plan's call, chosen for clear visual separation and to avoid any color reading as a shade of green/red:

| State | Color (DaisyUI token) |
|---|---|
| `supported` | `success` (green) |
| `contradicted` | `error` (red) |
| `partially_supported` | `warning` (amber) |
| `unsupported` | `neutral` / gray-brown, distinct from pending-gray |
| `unverifiable` | `info` (blue) |
| claim `status: pending` (not yet checked) | plain gray, subtle pulse animation — not a verdict color |
| claim `status: failed` (verification itself errored) | muted gray with a dashed outline — visually distinct from `unverifiable`, never conflated with it (FR-012) |
...
├── hooks/
│   ├── usePollGrounnelStatus.ts     # new — 10s poll loop
