...
- What happens when a claim's extracted text can't be confidently located inside the pasted article (paraphrase, resolved pronoun, near-original but not verbatim wording)? — Per FR-010, it still gets a best-effort fallback location rather than going unhighlighted, visually marked (dashed underline + tooltip note) as an approximate location rather than a confirmed match.
...
- What happens when a highlighted claim has zero sources? — The span still gets its verdict color per FR-007. For `partially_supported`/`contradicted` (verdict implies at least one citation existed) this stays tooltip-free. For a zero-evidence verdict (`unsupported`/`unverifiable`) with no sources or citations at all, the tooltip instead shows an explicit "no sources were found to check this claim" note (revised 2026-08-16, user-reported confusion — a bare, unexplained icon with nothing to hover read as broken) rather than either an empty tooltip or none at all.
- What happens before the first status poll returns, when a run exists but progress data (checked/total) hasn't arrived yet? — The progress indicator shows an indeterminate "finding claims…" state rather than a numeric `0 / 0`, since the total claim count isn't known until extraction completes.
...
- **FR-006**: System MUST redisplay the submitted article text below the textarea, with only the spans matched to a claim visually highlighted — all other text MUST remain unstyled. "Matched span" may be the claim's exact wording, a containing sentence/clause identified as the claim's likely source, or — when nothing clears the normal confidence bar — a best-effort fallback location (see FR-010) so every claim always has somewhere to point to; either way, no unrelated surrounding text is ever highlighted.
- **FR-007**: System MUST color each highlighted span according to its claim's verdict: `supported` = green, `contradicted` = red; `partially_supported`, `unsupported`, and `unverifiable` MUST each have their own visually distinguishable color, none of which is confusable with green or red. Verdict distinction MUST NOT rely on color alone — each verdict MUST also be identifiable via a non-color cue (e.g. an icon, label, or pattern) available on hover/focus, so highlighting stays usable for color-vision-deficient users.
...
- **FR-009**: System MUST display a References list below the article, numbered Wikipedia-style — one entry per unique cited source (not per claim; a source cited by two different claims reuses its number), each linking to the exact cited sentence on the source page where possible. Claims with no resolvable citation are not listed there (revised 2026-08-12 — the earlier "Not independently sourced" grouped section was removed; every claim is instead located in the article body itself, see FR-010).
- **FR-010**: System MUST locate every claim SOMEWHERE in the article body (revised 2026-08-12, user-requested — previously a claim whose text couldn't be confidently located was left unhighlighted; every claim now always gets a span). Location confidence is tiered — exact wording, then a containing sentence, then a clause within a compound sentence, then a best-effort no-threshold fallback (ranked by relevance, skipping any span already taken by a higher-confidence claim). The lowest (no-threshold fallback) tier MUST be visually distinguishable from a confirmed match (revised 2026-08-12 — a real run showed an unrelated claim's fallback guess landing on a date-heavy sentence and reading as if its verdict were about that date; the fallback tier now gets a dashed underline (matching the existing dashed "not a confirmed verdict" convention used for a failed/pending claim) plus an "approximate location" note in its tooltip, on top of its normal verdict color/icon). Overlap is still resolved by confidence tier first, then earliest position; a claim can still end up unhighlighted only in genuine oversubscription (more claims than the article has non-conflicting sentence/clause slots for).
...
- **FR-013**: System MUST disable the Run button and input for the full duration of a run (from submit until its status reaches `done`/`failed`), so a second run can never overlap the first (revised 2026-08-12, user-requested reversal of the original "submit at any time" behavior — letting two runs overlap made results from both appear to blend together in the UI). Submitting a new run still fully cancels any previous run's polling and resets the displayed results to only the new run's state (unchanged from before, just now unreachable mid-run since the button is disabled).
...
- **SC-004**: Verdict states are distinguishable from each other at a glance and without relying on color perception alone (per FR-007) — 5 distinct colors for the 5 verdicts, plus separate, mutually distinguishable neutral treatments for a claim still being checked (`pending`) and a claim whose verification itself errored (`status: failed`), per FR-012.
