...
- What happens when a highlighted claim has zero sources? — The span still gets its verdict color per FR-007, but has no tooltip (FR-008's tooltip only applies when at least one source exists) — hovering it does nothing, rather than showing an empty tooltip.
