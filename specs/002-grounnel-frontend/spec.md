...
- **FR-006**: System MUST redisplay the submitted article text below the textarea, with only the spans matched to a claim visually highlighted — all other text MUST remain unstyled. "Matched span" may be the claim's exact wording OR, when exact wording can't be located, the single containing sentence identified as the claim's likely source (see FR-010 for when no match is found at all) — either way, no unrelated surrounding text is ever highlighted.
...
- **FR-009**: System MUST display a References list below the article, numbered Wikipedia-style — one entry per unique cited source (not per claim; a source cited by two different claims reuses its number), each linking to the exact cited sentence on the source page where possible. Claims with no resolvable citation but real attempted sources MUST still show those source links, grouped separately ("Not independently sourced") rather than being hidden.
- **FR-010**: System MUST NOT highlight a claim in the article body when its text cannot be confidently located there (including via the clause-level fallback for compound sentences bundling multiple facts), while still surfacing that claim's verdict/sources/references below.
