...
- [x] T038 Fallback-tier visual distinction — resolves Phase 8's deferred finding. `matchClaimSpans.ts`: `MatchTier` and the tier of the winning match are now exported on each returned `Span` (`tier?: MatchTier`). `HighlightedArticle.tsx`: a `Fallback`-tier span gets a dashed underline (code-review finding: reuses the existing `FAILED_STYLE`/`UnconfirmedDot`/`FailedDot` dashed "not confirmed" idiom instead of a one-off dotted convention) plus an "Approximate location — this text wasn't confirmed as a close match" note in its tooltip (forced open even with zero sources, so the disclaimer is reachable), on top of its normal verdict color/icon — so the coverage guarantee ("every claim highlighted somewhere," T031) no longer implies precision a fallback guess doesn't have. New tests in `matchClaimSpans.test.ts` assert `tier` is `Exact`/`Fallback` as expected.
